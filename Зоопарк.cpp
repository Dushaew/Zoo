...
void addAnimal(vector<Animal*>& animals) {
    ...
    if (type == 'F') {
        bool isSaltwater;
        cout << "Is the fish saltwater (Yes/No)? ";
        char saltwater;
        cin >> saltwater;
        isSaltwater = (saltwater == 'Yes');
        bool isViviparous;
        cout << "Does the fish give birth to live young (Yes/No)? ";
        char viviparous;
        cin >> viviparous;
        isViviparous = (viviparous == 'Yes');
        ...
        bool canFly;
        cout << "Can the bird fly (Yes/No)? ";
        char fly;
        cin >> fly;
        canFly = (fly == 'Y');
        bool canSing;
        cout << "Can the bird sing (Yes/No)? ";
        char sing;
        cin >> sing;
        canSing = (sing == 'Yes');
        ...
        cout << "Does the mammal have fur (Yes/No)? ";
        char fur;
        cin >> fur;
        hasFur = (fur == 'Y');
        cout << "Is the mammal a carnivore (Yes/No)? ";
        char carnivore;
        cin >> carnivore;
        isCarnivore = (carnivore == 'Yes');
...
int main() {
    ...
    animals.push_back(new Fish("Cyprinus", 0.5, "River", false, false));
    animals.push_back(new Fish("Shark", 2, "Ocean", true, false));
    animals.push_back(new Fish("Trout", 1, "Lake", true, true));

    animals.push_back(new Bird("Stork", 6, "Mountains", true, false));
    animals.push_back(new Bird("Penguin", 2, "Antarctica", false, false));
    animals.push_back(new Bird("Peacock", 7, "Asia", true, true));

    animals.push_back(new Mammal("Zebra", 1, "Savannah", true, true));
    animals.push_back(new Mammal("Whale", 3, "Ocean", false, true));
    animals.push_back(new Mammal("Pig", 6, "Russia", true, false));
